...
@section('content')
                        ...
                        <div class="flex flex-col gap-8 lg:col-span-5">
                            @foreach ($featuredPosts->skip(1) as $post)
                                <article class="group flex gap-4 border-t border-stone-100 pt-6 first:border-t-0 first:pt-0">
                                    <a href="{{ route('posts.show', $post->slug) }}" class="w-28 shrink-0 overflow-hidden sm:w-36">
                                        <x-cover-image
                                    ...
                                    <div>
                                        ...
                                        @endif
                                        <h3 class="mt-1 font-display text-lg leading-snug text-stone-900">
    ...
    @else
        <section class="border-b border-stone-200 bg-white">
            <x-site-container class="max-w-6xl py-16 sm:py-20">
                ...
                <h1 class="mt-4 max-w-2xl font-display text-4xl leading-tight text-stone-900 sm:text-5xl">
        ...
        <section>
            <x-site-container class="max-w-6xl py-14 sm:py-16">
                <div class="flex items-end justify-between gap-4">
                    <h2 class="font-display text-3xl text-stone-900">Son Yazılar</h2>
                    <a href="{{ route('posts.index') }}" class="text-sm font-medium uppercase tracking-widest text-accent-700 hover:text-accent-800">Tümünü gör</a>
                </div>
    ...
    @if ($categories->isNotEmpty())
        <section class="border-t border-stone-200 bg-white">
            <x-site-container class="max-w-6xl py-14 sm:py-16">
                <h2 class="font-display text-3xl text-stone-900">Kategoriler</h2>
                <div class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($categories as $category)
                        ...
                        >
                            <h3 class="font-display text-xl text-stone-900 group-hover:text-accent-800">{{ $category->name }}</h3>
